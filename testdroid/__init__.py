...
Consider using start_test_run_using_config() instead.
    ...
    def print_builds(self, job_id, limit=0):
        ...
        for build in self.get_builds(job_id, limit)['data']:
            print("%s %s %s %s %s" % (str(build['id']).ljust(12), str(build['buildNumber']).ljust(5),
                                      build['state'].ljust(10), build['status'].ljust(10), build['duration']))
    ...
    def create_build(self, job_id, build_config={}):
        build = self.post(path="me/jobs/{}/builds".format(job_id), payload=build_config,
    ...
    def download_build_output_files(self, job_id, build_id, results_folder="results", tags=None):
        files = self.get("me/jobs/{}/builds/{}/output-file-set/files{}".format(
            job_id, build_id, "?tag[]=".format(tags) if tags else ""))
        self.__download_files(files, results_folder)
        ...
        if job_id and build_id:
            ...
            while True:
                ...
                if not self.api_key:
                    self.access_token = None
                    self.get_token()
                build_status = self.get_build(job_id, build_id)
                if build_status and 'state' in build_status:
                    if build_status['state'] == "FINISHED":
                        print("The build with id: %s has FINISHED with status: %s" % (build_id, build_status['status']))
                        break
                    elif build_status['state'] == "CREATED":
                        print("[%s] The build with id: %s is awaiting to be scheduled" %
                              (time.strftime("%H:%M:%S"), build_id))
                        continue
                    elif build_status['state'] == "BUILDING":
...
Commands:
    ...
    set-project-config <project-id> <config-json>
                                                ***DEPRECATED*** 
                                                ...
                                                e.g.:
                                                ./testdroid-api-client set-project-config 1234 
                                                '{"limitationType":"CLASS", "limitationValue":"com.foo.test.VerifyFoo"}'
    ...
    create-job <job-name> <job-configuration>   Create a new job. Job configuration in Jenkins pipeline format
                                                See the sample of Jenkisfile in 
                                                http://docs.bitbar.com/build-service/guide.html
    update-job <job-id> <job-name> <job-configuration>
                                                Update existing job
    create-build <job-id> <build-configuration> Create a new build job. 
                                                See https://cloud.bitbar.com/cloud/swagger-ui.html
